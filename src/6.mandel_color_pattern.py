...
width = 1000
height = 1000
colour_map = ColourMap.create_linear_palette(ColourMap.red, ColourMap.yellow, 16)
bitmap = Image.new("RGB", (width, height), "white")
start = time.time()
re, rm, n = mandelbrot_set(-2.0, 0.5, -1.25, 1.25, width, height, 400)
print("执行时间 {} 秒".format(round(time.time() - start, 2)))
start = time.time()

# nr = np.frompyfunc(matrix_p, 1, 1)(n)
nr = np.frompyfunc(get_color(colour_map), 1, 1)(n)
